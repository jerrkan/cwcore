            ...
            if(QueryResult *loginfail = loginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str()))
            {
                ...
                if( failed_logins >= MaxWrongPassCount )
                {
                    ...
                    if(WrongPassBanType)
                    {
                        ...
                        loginDatabase.PExecute("INSERT INTO account_banned VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','CW realmd','Failed login autoban',1)",
                    ...
                    else
                    {
                        ...
                        loginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','CW realmd','Failed login autoban')",
