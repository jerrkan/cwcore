...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleNULL,                                      //262 ignore combat/aura state?    &Aura::HandleAuraAllowOnlyAbility,                      //263 SPELL_AURA_ALLOW_ONLY_ABILITY player can use only abilities set in SpellClassMask
	...
	&Aura::HandleNULL,                                      //273 x-ray effect    &Aura::HandleNoImmediateEffect,                         //274 SPELL_AURA_CONSUME_NO_AMMO implemented in spell::CalculateDamageDoneForAllTargets
	...
	&Aura::HandleUnused,                                    //284 51 spells    &Aura::HandleAuraModAttackPowerOfArmor,                 //285 SPELL_AURA_MOD_ATTACK_POWER_OF_ARMOR  implemented in Player::UpdateAttackPowerAndDamage
	...
	&Aura::HandleUnused,                                    //288 increase parry/deflect, prevent attack    &Aura::HandleNULL,                                      //289 unused
    ...
    if(GetParentAura()->GetSourceGUID() == m_target->GetGUID())
    {
        ...
        if( !source->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            switch(m_areaAuraType)
            {
                ...
                {
                    CW::AnyFriendlyUnitInObjectRangeCheck u_check(source, caster, m_radius);
                    CW::UnitListSearcher<CW::AnyFriendlyUnitInObjectRangeCheck> searcher(source, targets, u_check);
                    source->VisitNearbyObject(m_radius, searcher);
                    break;
                }
                case AREA_AURA_ENEMY:
                {
                    CW::AnyAoETargetUnitInObjectRangeCheck u_check(source, caster, m_radius); // No GetCharmer in searcher
                    CW::UnitListSearcher<CW::AnyAoETargetUnitInObjectRangeCheck> searcher(source, targets, u_check);
                    source->VisitNearbyObject(m_radius, searcher);
                    break;
                }
...
void AuraEffect::HandleChannelDeathItem(bool apply, bool Real, bool /*changeAmount*/)
{
    if(Real && !apply)
    {
        ...
        if( spellInfo->EffectItemType[m_effIndex] == 6265 &&
            (victim->getLevel() <= CW::XP::GetGrayLevel(caster->getLevel()) ||
...
void AuraEffect::HandleFeignDeath(bool apply, bool Real, bool /*changeAmount*/)
{
    ...
    if( apply )
    {
        ...
        */
        ...
        std::list<Unit*> targets;
        CW::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_target, m_target, m_target->GetMap()->GetVisibilityDistance());
        CW::UnitListSearcher<CW::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_target, targets, u_check);
