...
void CreatureEventAIMgr::LoadCreatureEventAI_Texts()
{
    ...
    // Load EventAI Text
    objmgr.LoadCWStrings(WorldDatabase,"creature_ai_texts",MIN_CREATURE_AI_TEXT_STRING_ID,MAX_CREATURE_AI_TEXT_STRING_ID);
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; j++)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_QUEST_EVENT:
                        ...
                        {
                            if (!qid->HasFlag(QUEST_CW_FLAGS_EXPLORATION_OR_EVENT))
