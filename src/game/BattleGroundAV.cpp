...
{
    return CW::Honor::hk_honor_at_level(m_MaxLevel, kills);
}
...
void BattleGroundAV::HandleKillUnit(Creature *unit, Player *killer)
{
    ...
    else if(entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0])
    {
        ...
        if(creature)
            YellToAll(creature,GetCWString(LANG_BG_AV_A_CAPTAIN_DEAD),LANG_UNIVERSAL);
    ...
    else if ( entry == BG_AV_CreatureInfo[AV_NPC_H_CAPTAIN][0] )
    {
        ...
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if(creature)
            YellToAll(creature,GetCWString(LANG_BG_AV_H_CAPTAIN_DEAD),LANG_UNIVERSAL);
    }
...
void BattleGroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    char buf[256];
    if(IsTower(node))
        sprintf(buf, GetCWString(LANG_BG_AV_TOWER_TAKEN) , GetNodeName(node),( owner == ALLIANCE ) ? GetCWString(LANG_BG_AV_ALLY) : GetCWString(LANG_BG_AV_HORDE)  );
    else
        sprintf(buf, GetCWString(LANG_BG_AV_GRAVE_TAKEN) , GetNodeName(node),( owner == ALLIANCE ) ? GetCWString(LANG_BG_AV_ALLY) :GetCWString(LANG_BG_AV_HORDE)  );
...
//    }
    ...
    char buf[256];
        sprintf(buf, GetCWString(LANG_BG_AV_MINE_TAKEN), GetCWString(( mine == AV_NORTH_MINE ) ? LANG_BG_AV_MINE_NORTH : LANG_BG_AV_MINE_SOUTH), ( team == ALLIANCE ) ?  GetCWString(LANG_BG_AV_ALLY) : GetCWString(LANG_BG_AV_HORDE));
        Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
        if(creature)
            YellToAll(creature,buf,LANG_UNIVERSAL);
    }
...
void BattleGroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
   ...
   //spawn new go :)
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, GetCWString(( IsTower(node) ) ? LANG_BG_AV_TOWER_DEFENDED : LANG_BG_AV_GRAVE_DEFENDED), GetNodeName(node),( team == ALLIANCE ) ?  GetCWString(LANG_BG_AV_ALLY) : GetCWString(LANG_BG_AV_HORDE));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if(creature)
        YellToAll(creature,buf,LANG_UNIVERSAL);
...
void BattleGroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    //send a nice message to all :)
    char buf[256];
    sprintf(buf, ( IsTower(node) ) ? GetCWString(LANG_BG_AV_TOWER_ASSAULTED) : GetCWString(LANG_BG_AV_GRAVE_ASSAULTED), GetNodeName(node),  ( team == ALLIANCE ) ?  GetCWString(LANG_BG_AV_ALLY) : GetCWString(LANG_BG_AV_HORDE ));
    Creature* creature = GetBGCreature(AV_CPLACE_HERALD);
    if(creature)
        YellToAll(creature,buf,LANG_UNIVERSAL);
...
const char* BattleGroundAV::GetNodeName(BG_AV_Nodes node)
{
    switch (node)
    {
        case BG_AV_NODES_FIRSTAID_STATION:  return GetCWString(LANG_BG_AV_NODE_GRAVE_STORM_AID);
        case BG_AV_NODES_DUNBALDAR_SOUTH:   return GetCWString(LANG_BG_AV_NODE_TOWER_DUN_S);
        case BG_AV_NODES_DUNBALDAR_NORTH:   return GetCWString(LANG_BG_AV_NODE_TOWER_DUN_N);
        case BG_AV_NODES_STORMPIKE_GRAVE:   return GetCWString(LANG_BG_AV_NODE_GRAVE_STORMPIKE);
        case BG_AV_NODES_ICEWING_BUNKER:    return GetCWString(LANG_BG_AV_NODE_TOWER_ICEWING);
        case BG_AV_NODES_STONEHEART_GRAVE:  return GetCWString(LANG_BG_AV_NODE_GRAVE_STONE);
        case BG_AV_NODES_STONEHEART_BUNKER: return GetCWString(LANG_BG_AV_NODE_TOWER_STONE);
        case BG_AV_NODES_SNOWFALL_GRAVE:    return GetCWString(LANG_BG_AV_NODE_GRAVE_SNOW);
        case BG_AV_NODES_ICEBLOOD_TOWER:    return GetCWString(LANG_BG_AV_NODE_TOWER_ICE);
        case BG_AV_NODES_ICEBLOOD_GRAVE:    return GetCWString(LANG_BG_AV_NODE_GRAVE_ICE);
        case BG_AV_NODES_TOWER_POINT:       return GetCWString(LANG_BG_AV_NODE_TOWER_POINT);
        case BG_AV_NODES_FROSTWOLF_GRAVE:   return GetCWString(LANG_BG_AV_NODE_GRAVE_FROST);
        case BG_AV_NODES_FROSTWOLF_ETOWER:  return GetCWString(LANG_BG_AV_NODE_TOWER_FROST_E);
        case BG_AV_NODES_FROSTWOLF_WTOWER:  return GetCWString(LANG_BG_AV_NODE_TOWER_FROST_W);
        case BG_AV_NODES_FROSTWOLF_HUT:     return GetCWString(LANG_BG_AV_NODE_GRAVE_FROST_HUT);
        default:
            {
